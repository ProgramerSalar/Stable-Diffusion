...
class MergedRescaleDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.rescaler = LatentRescaler(factor=rescale_factor,
                                       ...
                                       mid_channels=temp_chn,
                                       out_channels=temp_chn,
                                       depth=rescale_module_depth
                                       )
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_timestep:
            ...
            self.temb.dense = nn.ModuleList([
                torch.nn.Linear(in_features=self.ch,
                                out_features=self.temb_ch),

                torch.nn.Linear(in_features=self.temb_ch,
                                out_features=self.temb_ch)
            ])
    ...
    def forward(self, x, t=None, context=None):
        ...
        if context is not None:
            x = torch.cat(tensors=(x, context), dim=1)

        if self.use_timestep:

            assert t is not None
            temb = get_timestep_embedding(timesteps=t,
...
class FirstStagePostProcessor(nn.Module):
    ...
    def __init__(self,
        ...
        if pretrained_config is None:
            ...
            self.instantiae_pretrained(pretrained_config)
        ...
        self.proj_norm = Normalize(in_channels=in_channels,
        ...
        for m in ch_mult:
            ...
            ch_in = m * n_channels
            downs.append(Downsample(in_channels=ch_in,
                                    with_conv=False))
    ...
    def instantiae_pretrained(self, config):
    ...
    def forward(self, x):
        ...
        for submodel, downmodel in zip(self.model, 
                                       self.downsampler):
...
# ----------------------------------------------------------------------------------------------
    ...
    # output = merged_rescale_encoder(x).to(device)
    
    # print(output.shape)  # torch.Size([4, 1024, 32, 32]), with LatentRescaler ---> torch.Size([4, 3, 64, 64])


    # merge_rescale_decoder = MergedRescaleDecoder(z_channels=3,
    #                                              out_ch=3,
    #                                              resolution=256,
    #                                              num_res_blocks=2,
    #                                              attn_resolutions=[],
    ...
    # print(merge_rescale_decoder)


# -----------------------------------------------------------------------------------------------------------

    # x = torch.randn(4, 128, 32, 32)
    # upsampler = Upsampler(in_size=32,
    #                       out_size=64,
    #                       in_channels=128,
    #                       out_channels=64)(x)
    
    # print(upsampler.shape)
